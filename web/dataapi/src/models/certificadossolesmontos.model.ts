...
@model({
  settings: {
    idInjection: false,
    db2: {schema: db2config.schema, table: 'CERTIFICADOSSOLESMONTOS'}
  }
})
export class Certificadossolesmontos extends Entity {
  @property({
    type: 'string',
    required: true,
    length: 15,
  ...
  constructor(data?: Partial<Certificadossolesmontos>) {
    super(data);
  }
}

export interface CertificadossolesmontosRelations {
  // describe navigational properties here
}

export type CertificadossolesmontosWithRelations = Certificadossolesmontos & CertificadossolesmontosRelations;
