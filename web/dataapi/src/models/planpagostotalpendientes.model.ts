...
@model({
  settings: {
    idInjection: false,
    db2: {schema: db2config.schema, table: 'PLANPAGOSTOTALPENDIENTES'}
  }
})
export class Planpagostotalpendientes extends Entity {
  @property({
    type: 'string',
    required: true,
    length: 5,
  ...
  constructor(data?: Partial<Planpagostotalpendientes>) {
    super(data);
  }
}

export interface PlanpagostotalpendientesRelations {
  // describe navigational properties here
}

export type PlanpagostotalpendientesWithRelations = Planpagostotalpendientes & PlanpagostotalpendientesRelations;
