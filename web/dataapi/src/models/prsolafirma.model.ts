...
@model({
  settings: {idInjection: false, db2: {schema: db2config.schema, table: 'PRSOLAFIRMA'}}
})
export class Prsolafirma extends Entity {
  @property({
    type: 'string',
    required: true,
    length: 5,
  ...
  constructor(data?: Partial<Prsolafirma>) {
    super(data);
  }
}

export interface PrsolafirmaRelations {
  // describe navigational properties here
}

export type PrsolafirmaWithRelations = Prsolafirma & PrsolafirmaRelations;
