...
@model({
  settings: {idInjection: false, db2: {schema: db2config.schema, table: 'PRLARGOPLAZO'}}
})
export class Prlargoplazo extends Entity {
  @property({
    type: 'string',
    required: true,
    length: 5,
  ...
  constructor(data?: Partial<Prlargoplazo>) {
    super(data);
  }
}

export interface PrlargoplazoRelations {
  // describe navigational properties here
}

export type PrlargoplazoWithRelations = Prlargoplazo & PrlargoplazoRelations;
