...
@model({
  settings: {idInjection: false, db2: {schema: db2config.schema, table: 'PRCONSUMO'}}
})
export class Prconsumo extends Entity {
  @property({
    type: 'string',
    required: true,
    length: 5,
  ...
  constructor(data?: Partial<Prconsumo>) {
    super(data);
  }
}

export interface PrconsumoRelations {
  // describe navigational properties here
}

export type PrconsumoWithRelations = Prconsumo & PrconsumoRelations;
